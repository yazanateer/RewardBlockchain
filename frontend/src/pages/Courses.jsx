    ...
    const fetchCourses = async () => {
      ...
      try {
        ...
        for (let i = 1; i <= totalCourses; i++) {
          ...
          const milestonesArray = Array.from(milestones);

          fetchedCourses.push({
            id: Number(id),
            name: title,
            description: description,
            milestones: milestonesArray.length,
            status: "not-registered", // default, can be updated later
          });
        }
          ...
          <div key={i} className="mb-12">
            ...
            <div className="relative">
              {filteredCourses.length > 2 && (
                <>
                  <button
                    className={`prev-btn-${i} absolute -left-8 top-1/2 transform -translate-y-1/2 z-10 text-3xl ${section.arrowColor} hover:scale-110 transition bg-white/10 border ${section.border} p-2 rounded-full shadow-lg backdrop-blur-md`}
                  >
                    ‹
                  </button>
                  <button
                    className={`next-btn-${i} absolute -right-8 top-1/2 transform -translate-y-1/2 z-10 text-3xl ${section.arrowColor} hover:scale-110 transition bg-white/10 border ${section.border} p-2 rounded-full shadow-lg backdrop-blur-md`}
                  >
                    ›
                  </button>
                </>
